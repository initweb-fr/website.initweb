...
export function manageNewsBanner() {
  const newsBanner = document.querySelector('.nav_banner');
  const closeButton = document.querySelector('.nav_banner_item_close');
  ...
  if (newsBanner) {
    ...
    if (bannerClosedTime && getCurrentTime() < bannerClosedTime) {
    ...
    if (closeButton) {
      ...
      closeButton.addEventListener('click', function () {
        newsBanner.style.display = 'none';
        localStorage.setItem('newsBannerClosedTime', getExpiryTime());
      });
    }
  }
}
